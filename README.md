...
Современный сайт психотерапевта на Next.js 16 с TinaCMS для редактирования контента, защищенной админ-зоной и полностью типизированной моделью данных.

Этот репозиторий устроен так, чтобы его было удобно поддерживать в долгую: публичная часть, административный вход, CMS-схема, контент-модель и механика защиты разделены по отдельным слоям.

## Кратко

- Публичный сайт для представления специалиста, услуг и контактов.
- TinaCMS для редактирования текста без ручного изменения кода.
...
- Русифицированный интерфейс админки Tina для комфортной работы.
...
- TinaCMS как система управления контентом.
- Поддержка локального режима разработки через `tinacms dev`.
...
- Русифицированный интерфейс Tina Admin.
